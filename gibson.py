...
class Window(object):
    kS_LIMBO, kS_EXPAND_W, kS_EXPAND_H, kS_PRINT, kS_SHRINK, kS_FINISH = (
        0,
        1,
        2,
        3,
    ...
    def random_frame(self):
        try:
            return (
                1,
                1,
                randint(self.kMIN_H, self.parent.height - self.kMIN_H),
                randint(self.kMIN_W, self.parent.width - self.kMIN_W),
            )
        except:
            return (
                1,
                1,
                randint(self.kMIN_ABS, self.parent.height - self.kMIN_ABS),
                randint(self.kMIN_ABS, self.parent.width - self.kMIN_ABS),
            )
    ...
    def stage_expand_w(self):
        self.w, self.x, at_max = self.expand_1D(
            self.w, self.x, self.parent.width, self.kRATE
        )
        self.update_window()

        self.win.border(1, 1, 1, curses.ACS_HLINE, 1, 1, 1, 1)
        if at_max or (self.w >= self.kMIN_W and randint(0, 10) == 0):
            self.stage = self.kS_EXPAND_H

    def stage_expand_h(self):
        self.win.erase()

        self.h, self.y, at_max = self.expand_1D(
            self.h, self.y, self.parent.height, self.kRATE
        )
        self.update_window()
    ...
    def shrink_1D(self, size, pos, bounds, rate):
        ...
        return (
            (size, pos, True)
            if new_pos < 0 or new_size <= bounds
            else (new_size, new_pos, False)
        )
    ...
    def expand_1D(self, size, pos, bounds, rate):
        ...
        return (
            (size, pos, True)
            if new_pos < 0 or new_pos + new_size > bounds
            else (new_size, new_pos, False)
        )

    kSTAGES = {
        kS_LIMBO: stage_limbo,
        kS_EXPAND_W: stage_expand_w,
        kS_EXPAND_H: stage_expand_h,
        kS_PRINT: stage_print,
        kS_SHRINK: stage_shrink,
        kS_FINISH: stage_finish,
    }
...
class SubWindow(object):
    ...
    def set_type(self):
        self.content = (
            ""
            if randint(0, 3) == 0
            else self.w * self.h * ("#" if randint(0, 5) == 0 else " ")
        )
        self.content_color = (
            curses.color_pair(randint(0, self.parent.parent.color_range))
            if self.parent.parent.random_colors
            else self.parent.parent.text_color
        )
        if randint(0, 4) == 0:
            self.content_color = self.content_color | curses.A_BOLD
        self.main_set, self.alt_set = sample(
            [
                "01",
                string.printable,
                string.ascii_letters + " " * 50,
                string.hexdigits + " " * 10,
            ],
            2,
        )
        ...
        self.lifespan = (
            randint(self.w, self.w * 2)
            if self.full_type == self.kTYPE_SCROLL
            else randint(self.w * 2, self.w * 4)
        )
    ...
    def update(self):
        ...
        if self.full or self.alt:
            ...
            if self.full_type == self.kTYPE_SCROLL:
                ...
                self.content += self.random_line(
                    self.main_set, range(randrange(1, self.w))
                )
                ...
                self.content = (
                    self.content[:target] + line + self.content[target + self.w :]
                )
        ...
        if self.lifespan <= 0 and not self.full:
            self.lifespan = 1
...
class Gibson(object):
    ...
    def __init__(self, stdscr, args=None):
        ...
        self.attributes = {
            Window.kS_LIMBO: 0,
            Window.kS_EXPAND_W: curses.color_pair(5),
            Window.kS_EXPAND_H: curses.color_pair(4),
            Window.kS_PRINT: curses.color_pair(3),
            Window.kS_SHRINK: curses.color_pair(2),
            Window.kS_FINISH: curses.color_pair(2),
        }
    ...
    def activate_window(self):
        if self.should_update or len(self.active_windows) < self.max_active_windows:
            if len(self.inactive_windows) == 0:
                self.inactive_windows.append(Window(self))
    ...
    def remove_windows(self):
        self.active_windows = [
            x for x in self.active_windows if x not in self.to_remove
        ]
    ...
    def debug_output(self):
        self.stdscr.addstr(
            0,
            0,
            "w:[{}], h:[{}] mawin:[{}] awin:[{}] iwin:[{}]".format(
                self.width,
                self.height,
                self.max_active_windows,
                len(self.active_windows),
                len(self.inactive_windows),
            ),
        )
    ...
    def view_resized(self):
        ...
        if self.height >= self.kMIN_H and self.width >= self.kMIN_W:
            self.max_active_windows = int(
                round(((self.height + self.width) / 2) * 0.05)
            )
            ...
            self.stdscr.addstr(
                0,
                0,
                "Window too small. h[{} vs {}] w[{} vs {}]".format(
                    self.height, self.kMIN_H, self.width, self.kMIN_W
                ),
            )
...
class Driver(object):
    ...
    def update(self):
        ...
        lower = key.lower()
        if key == "KEY_RESIZE":
            self.gibson.view_resized()
        elif key == self.kKEY_ESC or lower == "q":
            self.running = False
        elif lower == "v":
            self.gibson.verbose = not self.gibson.verbose

        elif lower == "p":
            self.paused = not self.paused
        elif key == "c":
            self.gibson.random_colors = not self.gibson.random_colors
        elif key == "C":
            self.gibson.text_color = self.gibson.random_color()
